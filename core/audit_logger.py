...
class AuditLogger:
    ...
    def log_auth(self, action: str, actor: str, result: str = 'success',
                 ip_address: str = '', **kwargs) -> AuditEntry:
        severity = AuditSeverity.INFO if result == 'success' else AuditSeverity.WARNING
        return self.log(AuditCategory.AUTH, action, actor=actor,
                        result=result, severity=severity,
                        ip_address=ip_address, details=kwargs)

    def log_scan(self, action: str, actor: str = 'system', target: str = '',
                 result: str = 'success', **kwargs) -> AuditEntry:
        return self.log(AuditCategory.SCAN, action, actor=actor,
                        target=target, result=result, details=kwargs)

    def log_exploit(self, action: str, actor: str = 'system', target: str = '',
                    result: str = 'success', **kwargs) -> AuditEntry:
        return self.log(AuditCategory.EXPLOIT, action, actor=actor,
                        target=target, result=result,
                        severity=AuditSeverity.CRITICAL, details=kwargs)

    def log_user(self, action: str, actor: str = 'system', target: str = '',
                 result: str = 'success', **kwargs) -> AuditEntry:
        return self.log(AuditCategory.USER, action, actor=actor,
                        target=target, result=result, details=kwargs)

    def log_config(self, action: str, actor: str = 'system',
                   result: str = 'success', **kwargs) -> AuditEntry:
        return self.log(AuditCategory.CONFIG, action, actor=actor,
                        result=result, severity=AuditSeverity.WARNING,
                        details=kwargs)
