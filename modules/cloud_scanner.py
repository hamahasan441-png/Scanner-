...
class CloudScannerModule:
    ...
    def _test_metadata_via_param(self, url, method, param, value):
        ...
        metadata_indicators = [
            ...
            'AccessKeyId', 'SecretAccessKey', 'security-credentials',
            'computeMetadata', 'vmId', 'subscriptionId',
            'droplet_id', 'region-id',
        ]
        ...
        for payload, cloud_provider in metadata_payloads:
            try:
                ...
                response = self.requester.request(
                    url, method, data=data, headers=extra_headers,
                )
                if not response:
                    continue
