...
from core.recon_arsenal import (
    ReconArsenal,
    AmassAdapter, HttpxAdapter, KatanaAdapter, DnsxAdapter,
    FfufAdapter, GauAdapter, WaybackurlsAdapter, GobusterAdapter,
    FeroxbusterAdapter, MasscanAdapter, RustscanAdapter, HakrawlerAdapter,
    ArjunAdapter, ParamSpiderAdapter, DirsearchAdapter,
)
...
class TestAmassAdapter(unittest.TestCase):
    ...
    def test_parse_json_valid(self):
        with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
            f.write('{"name":"sub.example.com","addresses":[{"ip":"1.2.3.4","cidr":"1.2.3.0/24","asn":12345,"desc":"ISP"}]}\n')
    ...
    def test_run_passive(self, _, mock_cmd):
        ...
        with patch.object(self.adapter, '_parse_json', return_value=({
            'total_subdomains': 1, 'total_addresses': 1,
            'subdomains': ['sub.example.com'], 'addresses': ['1.2.3.4'],
...
class TestKatanaAdapter(unittest.TestCase):
    ...
    def test_parse_output_jsonl(self):
        data = json.dumps({'request': {'endpoint': 'http://example.com/api', 'method': 'POST', 'source': 'form', 'tag': 'form', 'attribute': 'action'}})
...
class TestHakrawlerAdapter(unittest.TestCase):
    ...
    def test_run_success(self, _, mock_cmd):
        mock_cmd.return_value = (0, 'http://example.com/page\nhttp://example.com/app.js\nhttp://example.com/style.css\n', '', 2.0)
...
class TestDirsearchAdapter(unittest.TestCase):
    ...
    def test_parse_json_dict_format(self):
        data = {
            'http://example.com': [
                {'url': 'http://example.com/admin', 'status': 200, 'content-length': 1234, 'content-type': 'text/html', 'redirect': ''},
...
class TestReconArsenal(unittest.TestCase):
    ...
    def test_get_available_tools(self):
        ...
        for name in ['amass', 'httpx', 'katana', 'dnsx', 'ffuf', 'gau',
    ...
    def test_run_tool_dispatches(self):
        ...
        with patch.object(self.arsenal.amass, 'run', return_value=ToolResult(
            tool='amass', target='example.com', success=True
        )) as mock_run:
    ...
    def test_all_unavailable_tools_return_tool_result(self):
        ...
        with patch('shutil.which', return_value=None):
            adapters = [
                AmassAdapter(), HttpxAdapter(), KatanaAdapter(), DnsxAdapter(),
                FfufAdapter(), GauAdapter(), WaybackurlsAdapter(), GobusterAdapter(),
                FeroxbusterAdapter(), MasscanAdapter(), RustscanAdapter(),
                HakrawlerAdapter(), ArjunAdapter(), ParamSpiderAdapter(),
                DirsearchAdapter(),
            ]
