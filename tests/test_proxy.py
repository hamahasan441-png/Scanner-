...
def _make_entry(method='GET', url='http://example.com/', status=200,
                intercepted=False, modified=False, notes='',
                body='', resp_body='OK'):
    """Build a ProxyHistoryEntry for test convenience."""
    req = ProxyRequest(method=method, url=url, body=body,
                       client_address='127.0.0.1:9999')
    resp = ProxyResponse(status_code=status, body=resp_body, elapsed=0.01)
    return ProxyHistoryEntry(request=req, response=resp,
                             intercepted=intercepted, modified=modified,
                             notes=notes)
...
class TestProxyRequest(unittest.TestCase):
    ...
    def test_custom_values(self):
        r = ProxyRequest(method='POST', url='http://a.com',
...
class TestProxyResponse(unittest.TestCase):
    ...
    def test_custom_values(self):
        r = ProxyResponse(status_code=404, headers={'X': 'Y'},
                          body='nope', elapsed=1.23)
...
class TestProxyHistoryEntry(unittest.TestCase):
    ...
    def test_with_response(self):
        ...
        e = ProxyHistoryEntry(response=resp, intercepted=True, modified=True,
                              notes='test')
...
class TestHistoryFiltering(unittest.TestCase):
    ...
    def test_filter_combined(self):
        results = self.proxy.filter_history(url_pattern=r'a\.com', method='GET',
                                            status_code=200)
        self.assertEqual(len(results), 2)
...
class TestRequestRules(unittest.TestCase):
    ...
    def test_apply_header_rule(self):
        self.proxy.add_request_rule(
            {'match': r'BadToken', 'replace': 'GoodToken', 'scope': 'header'})
    ...
    def test_apply_body_rule(self):
        self.proxy.add_request_rule(
            {'match': r'secret', 'replace': 'REDACTED', 'scope': 'body'})
...
class TestResponseRules(unittest.TestCase):
    ...
    def test_apply_body_response_rule(self):
        self.proxy.add_response_rule(
            {'match': r'password', 'replace': '***', 'scope': 'body'})
    ...
    def test_apply_header_response_rule(self):
        self.proxy.add_response_rule(
            {'match': r'private', 'replace': 'public', 'scope': 'header'})
    ...
    def test_apply_status_response_rule(self):
        self.proxy.add_response_rule(
            {'match': r'.*', 'replace': '200', 'scope': 'status'})
    ...
    def test_status_rule_invalid_replacement(self):
        self.proxy.add_response_rule(
            {'match': r'.*', 'replace': 'not_a_number', 'scope': 'status'})
        resp = ProxyResponse(status_code=500)
        modified = self.proxy._apply_response_rules(resp)
...
class TestForwardUpstream(unittest.TestCase):
    ...
    def test_forward_success(self, mock_urlopen):
        ...
        req = ProxyRequest(method='GET', url='http://example.com/',
                           headers={'Accept': '*/*'})
    ...
    def test_forward_http_error(self, mock_urlopen):
        ...
        err = urllib.error.HTTPError(
            url='http://x.com', code=403, msg='Forbidden',
