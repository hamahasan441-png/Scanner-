...
class TestPointToPoint(_EngineCapture, unittest.TestCase):
    ...
    def test_point_to_point_enables_all_vuln_modules(self):
        ...
        vuln_modules = [
            ...
            'websocket', 'deserialization', 'osint', 'fuzzer',
        ]
    ...
    def test_point_to_point_enables_recon_modules(self):
        cfg = self._run_main(['--point-to-point'])
        recon_modules = [
            'recon', 'subdomains', 'tech_detect', 'dir_brute',
            'discovery', 'shield_detect', 'real_ip', 'passive_recon',
            'enrich', 'chain_detect', 'exploit_search', 'attack_map',
        ]
    ...
    def test_point_to_point_enables_exploitation_modules(self):
        cfg = self._run_main(['--point-to-point'])
        exploit_modules = [
            'shell', 'dump', 'os_shell', 'brute',
            'exploit_chain', 'auto_exploit',
        ]
    ...
    def test_point_to_point_enables_scapy_modules(self):
        cfg = self._run_main(['--point-to-point'])
        scapy_modules = [
            'scapy', 'stealth_scan', 'arp_discovery',
            'dns_recon', 'scapy_vuln_scan', 'scapy_attack_chain',
        ]
...
class TestScanPlannerScapyModules(unittest.TestCase):
    ...
    def test_scapy_modules_in_planner(self):
        ...
        for key in scapy_keys:
            self.assertIn(key, MODULE_DESCRIPTIONS,
                          f'{key} should be in MODULE_DESCRIPTIONS')
