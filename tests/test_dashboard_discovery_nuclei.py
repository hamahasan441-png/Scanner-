...
class TestDashboardPanels(unittest.TestCase):
    ...
    def test_dashboard_has_all_24_tabs(self):
        resp = self.client.get('/')
        expected_tabs = [
            'dashboard', 'scanner', 'pipeline', 'exploits',
            'exploit-intel', 'attackmap', 'shells', 'scans',
            'history', 'findings', 'rules', 'livefeed',
            'auth', 'scheduler', 'compliance', 'audit',
            'tools', 'plugins', 'notifications', 'recon-arsenal',
            'discovery', 'nuclei', 'chat', 'ai-brain',
        ]
