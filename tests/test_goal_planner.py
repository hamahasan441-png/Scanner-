...
class TestGoalDataclass(unittest.TestCase):
    def test_creation(self):
        ...
        g = Goal(id='GOAL_0', claim='test', confidence=0.8,
                 target_endpoint='https://example.com', vuln_class='sqli',
                 required_tools=['sqli'], priority=1.0)
...
class TestGoalPlanner(unittest.TestCase):
    ...
    def test_plan_sorted_by_priority(self):
        ...
        for i in range(len(self.planner.goals) - 1):
            self.assertGreaterEqual(self.planner.goals[i].priority,
                                    self.planner.goals[i+1].priority)
    ...
    def test_push_goal(self):
        from core.goal_planner import Goal
        g = Goal(id='NEW', claim='new', confidence=0.9,
                 target_endpoint='t', vuln_class='v',
                 required_tools=[], priority=10.0)
